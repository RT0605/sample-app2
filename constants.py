...
SYSTEM_PROMPT_NOTICE_SLACK = """
    ...
    # 命令
    Slackの「動作検証用」チャンネルで、メンバーが{name_text}のメンバーに一度だけメンションを当て、生成したメッセージを送信してください。

    # 送信先のチャンネル名
    動作検証用

    # メッセージの通知先
    メンバーが{name_text}のメンバー

    # メッセージ通知（メンション付け）のルール
    - メンション付けはメッセージの先頭で「一度だけ」行ってください。
    - メンション付けの行は、メンションのみとしてください。

    # メッセージの生成条件
    - 各項目について、できる限り具体的に生成してください。
    ...
    - 「メンション先の選定理由」は、選ばれた従業員ごとの専門知識や過去の対応経験を具体的に記載してください。
